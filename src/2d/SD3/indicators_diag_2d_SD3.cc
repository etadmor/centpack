...
//
// indicators_diag.cc -- a core function of CentPack
//
...
//     indicators along the SW-NE and SE-NW directions:
//     
//     dISl  --  SE reconstruction
//
...
void CENTPACK::indicators_diag_2d_SD3(const doublearray3d& un, doublearray2d& dISl, doublearray2d& dIScx, doublearray2d& dISr, doublearray2d& dISb, doublearray2d& dIScy, doublearray2d& dISt, const doublearray1d& u_norm)
{
	...
	for (k=2; k<K+2; k++)
	{
		for (j=2; j<J+2; j++)
		{
			...
			dISl(j,k)=(1.0/L)*sum_2d_SD3(ISl_vector);
			dIScx(j,k)=(1.0/L)*sum_2d_SD3(IScx_vector);
			dISr(j,k)=(1.0/L)*sum_2d_SD3(ISr_vector);
			
			dISb(j,k)=(1.0/L)*sum_2d_SD3(ISb_vector);
			dIScy(j,k)=(1.0/L)*sum_2d_SD3(IScy_vector);
			dISt(j,k)=(1.0/L)*sum_2d_SD3(ISt_vector);
		}
	}
}
