...
int CENTPACK::centpack_main_1d_FD2()
{
	disclaimer();
	...
	do
	{
		...
		evolution_1d_FD2(un, uj_half, lambda, dx, gamma, alpha, B1, odd);
		
		dt_cpu=(clock()-t_start)/CLOCKS_PER_SEC;
		sum_t=sum_t+dt_cpu;
		
		end_of_step_1d_FD2(un, dt, t, dt_out, t_out, n, gamma, sum_t, dt_cpu, B1, odd);
